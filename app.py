...
def show_accessoires():
    ...
    with tab1:
        ...
        all_accessoires = get_all_records('accessoires')
        
        # Filtrage en mémoire
        if search_term:
            search_term_lower = search_term.lower()
            all_accessoires = [
                a for a in all_accessoires 
                if search_term_lower in a.get('designation', '').lower() or search_term_lower in a.get('reference', '').lower()
            ]
            
        if not all_accessoires:
            st.info("Catalogue vide. Ajoutez vos accessoires courants (clips, rivets, joints...) !")
        else:
            data_to_display = []
            for a in all_accessoires:
                prix_achat = float(a.get('prix_achat', 0.0))
                prix_vente = float(a.get('prix_vente', 0.0))
                quantite = int(a.get('quantite', 0))
                seuil_alerte = int(a.get('seuil_alerte', 0))
                
                marge_dzd = prix_vente - prix_achat
                marge_pct = round(((prix_vente - prix_achat) / prix_achat) * 100, 1) if prix_achat > 0 else 0.0
    ...
    with tab3:
        ...
        if not all_accessoires:
            ...
            art_dict = []
            for a in all_accessoires:
                display = f"{a.get('reference', '')} - {a.get('designation', '')} (Stock: {a.get('quantite', 0)}) [ID:{a.get('id', '')}]"
                art_dict.append(display)
            ...
            with col1:
                ...
                if st.button("📥 Valider Entrée", key="btn_acc_in"):
                    if qty_add > 0:
                        item = get_record('accessoires', art_id)
                        if item is None:
                            st.error("Accessoire introuvable.")
                            return
                            
                        new_qty = int(item.get('quantite', 0)) + int(qty_add)
                        update_record('accessoires', art_id, {"quantite": new_qty})
            ...
            with col2:
                ...
                if st.button("📤 Valider Sortie", key="btn_acc_out"):
                    ...
                    current_qty = int(item.get('quantite', 0))
    ...
    # --- TAB 4 : STATISTIQUES ---
    with tab4:
        st.subheader("📊 Statistiques des Accessoires")
        all_accessoires = get_all_records('accessoires')
        
        if not all_accessoires:
            st.info("Aucune donnée statistique disponible.")
        else:
            df_stats = pd.DataFrame(all_accessoires)
            # Sécurisation des types numériques
            df_stats['prix_achat'] = pd.to_numeric(df_stats.get('prix_achat', 0), errors='coerce').fillna(0)
            df_stats['prix_vente'] = pd.to_numeric(df_stats.get('prix_vente', 0), errors='coerce').fillna(0)
            df_stats['quantite'] = pd.to_numeric(df_stats.get('quantite', 0), errors='coerce').fillna(0)
            df_stats['seuil_alerte'] = pd.to_numeric(df_stats.get('seuil_alerte', 0), errors='coerce').fillna(0)
            
            df_stats['Marge_dzd'] = df_stats['prix_vente'] - df_stats['prix_achat']
            
            st.markdown("##### 💰 Top Marge : Les accessoires qui rapportent le plus")
            df_sorted_marge = df_stats.sort_values(by='Marge_dzd', ascending=False).head(10)
            
            fig_marge = px.bar(
                df_sorted_marge, x='designation', y='Marge_dzd', 
            ...
            st.markdown("---")
            st.markdown("##### ⚠️ Accessoires à commander urgemment (Stock Faible)")
            alertes = df_stats[df_stats['quantite'] <= df_stats['seuil_alerte']]
            
            if not alertes.empty:
                df_alertes = alertes[['reference', 'designation', 'quantite', 'seuil_alerte']].copy()
                df_alertes.columns = ['Référence', 'Désignation', 'Qté', 'Seuil Alerte']
                st.dataframe(df_alertes, use_container_width=True, hide_index=True)
            else:
...
if "veh_id" in query_params:
    try: show_qr_dashboard(int(query_params["veh_id"]))
    except: st.error("❌ Erreur URL")
else:
    try:
        ...
        elif module_name == "stock": show_stock()
        elif module_name == "accessoires": show_accessoires()
        elif module_name == "fournisseurs": show_fournisseurs()
        elif module_name == "achats": show_achats()
        elif module_name == "facturation": show_facturation()
        elif module_name == "caisse": show_caisse()
        elif module_name == "photos": show_photos()
        elif module_name == "employes": show_employes()
        elif module_name == "documents": show_documents()
        elif module_name == "statistiques": show_statistiques()
